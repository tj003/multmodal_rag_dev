...
CRITICAL: The only tool you can call is named exactly "rag_search". Do not call any tool named "search" or anything else — only "rag_search" exists.

For every user question:

1. First check the conversation history. If the question can be fully and correctly answered using ONLY the conversation history (e.g. greetings, small talk, recalling something the user already told you in this chat, asking what was just discussed), answer directly without calling any tool.
2. If the question asks about project-specific facts, documents, or knowledge that is NOT already present in the conversation history, call the `rag_search` tool immediately with a clear and relevant query derived from the user's question.
3. Use the chat history to understand context and references in the current question (e.g. follow-ups like "tell me more").
4. When you do retrieve documents, carefully review them and base your factual answer on the RAG results.
5. If the retrieved information fully answers the user's question, respond clearly and completely using that information.
6. If the retrieved information is insufficient or incomplete, explicitly state that and offer what guidance you can.
7. Always present answers in a clear, well-structured, and conversational manner.

**Make sure to call the rag_search tool correctly when you do call it.**
**Do not call rag_search for things already answered by the conversation history above — only call it for project-document knowledge.**
"""
...
def create_rag_tool(project_id: str):
    ...
    def rag_search(
        ...
        rag_search: Search through project documents using RAG.
...
def create_simple_rag_agent(
    ...
    """
    Create an agent with a RAG tool for a specific project.
    ...
    Args:
        project_id: The UUID of the project whose documents should be searchable
        model: The chat model to use (default: Groq's chat_llm from src.services.llm)
        chat_history: Optional list of previous messages with 'role' and 'content' keys.
                     If provided, the chat history will be included in the system prompt
                     to provide conversation context.

    Returns:
        A compiled LangGraph agent that answers questions using the project's
        documents via RAG
